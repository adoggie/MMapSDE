						...
						int *pnPtNum, int nObjType)
{
	...
	*pnPtNum = nNum;
	for(nSide = 1; nSide <= 4; nSide++) //检测矩形四个面内的点
	{
...
{
	double	dOff = (dy-MAP_ORIGIN_CENTERY)*12.0; //纬度方向网格块是5分为一个单位
...
//根据点位置得到图幅的ID,获取与当前可视地理区域相交的网格编号列表
//地理区域内查询出现的所有图块编号
void CalculateBMapIDInRect( ST_GEO_RECT rt, int nBMIDs[3000], int &nBNum )
{
	...
	int nBottom = 0;
	int nRight = 0;
	CalculateBMapIDInfo( rt.dLeft, rt.dTop, nLeft, nTop );  //计算两个对角定点所处于的数据存储区块编号
	CalculateBMapIDInfo( rt.dRight, rt.dBottom, nRight, nBottom );
	for( int i=nLeft;	i<=nRight;	i++ ) //扫描每个区块数据
	{
		for( int j=nTop;	j<=nBottom;	j++ )
		{
			nBMIDs[nBNum] = CalculateBMapID( i, j ); //根据xy的位置杂凑出图块的编号
			nBNum++;
		}
	}
}
...
stuVoiceTokenItem VoiceTokenTable[] = 
{
    ...
    {17,_T("存在与出口道路同名的道路(除入口道路外):是")},
    {18,_T("存在与出口道路同名的道路(除入口道路外):否")},
    {19,_T("存在与入口道路同名的道路(除出口道路外):是")},
    {20,_T("存在与入口道路同名的道路(除出口道路外):否")},
    {21,_T("是否存在除出入口道路外的其他道路:是")},
    {22,_T("是否存在除出入口道路外的其他道路:否")},
    {23,_T("是否存在IMI:是")},
    {24,_T("是否存在IMI:否")},
    {25,_T("是否导到目的地:是")},
    {26,_T("是否导到目的地:否")},
    {27,_T("拐弯方向:左")},
    {28,_T("拐弯方向:右")},
    {29,_T("入口道路与出口道路之间是否有环岛:是")},
    {30,_T("入口道路与出口道路之间是否有环岛:否")},
    {31,_T("入口道路和出口道路是否为同一条道路:是")},
    {32,_T("入口道路和出口道路是否为同一条道路:否")},
    {35,_T("是否为收费路段:是")},
    {36,_T("是否为收费路段:否")},
...
{  
    ...
    TRACE0("\t\t\t同名");VOICE_TRACE(byConfusion);
    TRACE0("\t\t\t异名");VOICE_TRACE(byConfusion2);
