// NetworkFileDataV7.cpp: implementation of the CNetworkFileDataV7 class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "NetworkFileDataV7.h"
...
CNetworkFileDataV7::CNetworkFileDataV7()
{
...
BOOL CNetworkFileDataV7::Load(LPCTSTR strFilePath)
{
#ifdef ANALYSE_DATA
...
//BOOL CNetworkFileDataV7::ReSort()
//{
...
//        
//        
//        StuNetworkFileStructV7::StuBreadthHeaderInfo* pBreadthHeader = pItem->pHeader;
//        
//        {
//            int nIndexN = 0;
...
//            {
//                pair <unsigned long ,CGIS_NetworkRoadSectionV7* > aPair = *iteratorOfFtr;
//                CGIS_NetworkRoadSectionV7* pFtr = (CGIS_NetworkRoadSectionV7*)aPair.second;
//                
...
//            iteratorOfFtr != pFtrContainer->end(); iteratorOfFtr++)
//            {
//                pair <unsigned long ,CGIS_NetworkVertexV7* > aPair = *iteratorOfFtr;
//                CGIS_NetworkVertexV7* pFtr = (CGIS_NetworkVertexV7*)aPair.second;
//                
...
//按V7格式写入文件
BOOL CNetworkFileDataV7::Write(LPCTSTR strFileName)
{
    //////////////////////////////////////////////////////////////////////////
	//写文件
	CMemFile aMemFile;
...
#ifdef _DEBUG
void CNetworkFileDataV7::DumpState()
{

    TRACE0("********************************************************************\n");
    TRACE0("CNetworkFileDataV7 Info:\n");
...
void CNetworkFileDataV7::AssertValid() const
{
...
BOOL CNetworkFileDataV7::WriteNetwork(CFile* pFile)
{
	//---------------------------
	//构造头,写头	
    ...
    StuNetworkFileStructV7::StuIndexInfoOfMap* pIndexInfoOfMap = 
        new StuNetworkFileStructV7::StuIndexInfoOfMap[nCount];

    //////////////////////////////////////////////////////////////////////////
    DWORD dwIndexPos = pFile->GetPosition();
    pFile->Write(pIndexInfoOfMap,sizeof(StuNetworkFileStructV7::StuIndexInfoOfMap) * nCount);
    //////////////////////////////////////////////////////////////////////////
    ...
    for(int i = 0; i < nCount; i++)
    {
        ...
        pItem->pIndex = new StuNetworkFileStructV7::StuIndexInfoOfMap;
        ...
        memcpy(&pIndexInfoOfMap[i],pItem->pIndex,sizeof(StuNetworkFileStructV7::StuIndexInfoOfMap));
    }
...
BOOL CNetworkFileDataV7::WriteMap       (CFile* pFile,stuMapItem* pMapItem)
{
    ASSERT(pMapItem);
    ASSERT(pMapItem->pIndex);
    ASSERT(pMapItem->pBreadthContainer);
    
    StuNetworkFileStructV7 :: StuIndexInfoOfMap* pIndexInfoOfMap = pMapItem->pIndex;
        
    int nCount = pIndexInfoOfMap->nBreadthCount;
    StuNetworkFileStructV7::StuIndexInfoOfBreadth* pIndexInfoOfBreadth 
        = new StuNetworkFileStructV7::StuIndexInfoOfBreadth[nCount];
    
    //写入所有图幅的索引
    //////////////////////////////////////////////////////////////////////////
    DWORD dwIndexPos = pFile->GetPosition();
    pFile->Write(pIndexInfoOfBreadth,sizeof(StuNetworkFileStructV7::StuIndexInfoOfBreadth)*nCount);
    //////////////////////////////////////////////////////////////////////////
    ...
    iteratorOfBreadth != pBreadthContainer->end(); iteratorOfBreadth++)
    {
        ...
        pItem->pIndex = new StuNetworkFileStructV7::StuIndexInfoOfBreadth;
        ...
        memcpy(&pIndexInfoOfBreadth[nIndexN],pItem->pIndex,sizeof(StuNetworkFileStructV7::StuIndexInfoOfBreadth));
        nIndexN++;
    }
...
BOOL CNetworkFileDataV7::WriteBreadth   (CFile* pFile,stuBreadthItem* pBreadthItem)
{
    ASSERT(pFile);
    ...
    StuNetworkFileStructV7 :: StuIndexInfoOfBreadth* pIndexInfoOfBreadth = pBreadthItem->pIndex;
    ...
    StuNetworkFileStructV7::StuBreadthHeaderInfo aBreadthHeaderInfo; 
	...
	CMemFile aMemFile;
	{
		...
		pFile->Write(&aBreadthHeaderInfo,sizeof(StuNetworkFileStructV7::StuBreadthHeaderInfo));		
        //////////////////////////////////////////////////////////////////////////
		//写入所有路段
		{
            MapOfRoadSection::iterator iteratorOfFtr;
            ...
            {
                pair < unsigned long,CGIS_NetworkRoadSectionV7*> aPair = *iteratorOfFtr;
                CGIS_NetworkRoadSectionV7* pFtr = aPair.second;
                unsigned long nIndex = aPair.first;
                ASSERTVALID(pFtr);
                pFile->Write(pFtr->GetDataAddr(),sizeof(StuNetworkFileStructV7::stuRoadSection));
		...
		//写入所有节点
		{
            ...
            {
                pair < unsigned long,CGIS_NetworkVertexV7*> aPair = *iteratorOfFtr;
                CGIS_NetworkVertexV7* pFtr = aPair.second;
                unsigned long nIndex = aPair.first;
                ASSERTVALID(pFtr);
                pFile->Write(pFtr->GetDataAddr(),sizeof(StuNetworkFileStructV7::stuVertex));
            }
        }
...
BOOL CNetworkFileDataV7::GetBuddyDataOfRoadSectionSize(
...
BOOL CNetworkFileDataV7::CollectBuddyDataOfRoadSection(
        MapOfRoadSection* pFtrContainer,
        char* pBuddy,
        unsigned long& nCharDataBuddyLength,
        unsigned long& nShortDataBuddyLength,
...

{
	...
	//对路段的处理
	{
        ...
        *( (DWORD*)pBuddyData ) = ('y'<<24|'l'<<16|'d'<<8|' '<<0);
        ...
        nShortDataBuddyLength = nCountOfBuddyData;   
        pBuddyData = NULL;
    }
	return TRUE;
}

BOOL CNetworkFileDataV7::GetBuddyDataOfVertexSize(
       ...
       unsigned long& nBuddyDataSize)
{
    ...
    {  
        ...
        iteratorOfFtr != pFtrContainer->end(); iteratorOfFtr++)
        {
            ...
            nNum = pFtr->GetLowLayerRoadSectionNum();
            ASSERT(nNum < 11);
            if (nNum > 0)
            {
                nSize = sizeof(unsigned short) * nNum;
    ...
    {  
        ...
        iteratorOfFtr != pFtrContainer->end(); iteratorOfFtr++)
        {
            ...
            if(pFtr->GetEdge())
            {
                CGIS_NetworkVertexV7::ST_GEO_VEDGE* pEdgeInfo = (CGIS_NetworkVertexV7::ST_GEO_VEDGE*)pCurrent;
                pCurrent += sizeof(CGIS_NetworkVertexV7::ST_GEO_VEDGE);
            }								
        }	
        int nCountOfBuddyData = pCurrent - pBuddyData;
        ASSERT(nCountOfBuddyData >= 4);
...
BOOL CNetworkFileDataV7::CollectBuddyDataOfVertex(
        MapOfVertex* pFtrContainer,
        char* pBuddy,
        unsigned long& nCharDataBuddyLength,
        unsigned long& nShortDataBuddyLength,
    ...
    *( (DWORD*)pBuddyData ) = ('y'<<24|'l'<<16|'d'<<8|' '<<0);
    ...
    {  
        ...
        iteratorOfFtr != pFtrContainer->end(); iteratorOfFtr++)
        {
            ...
            {
                nSize = sizeof(unsigned short) * nNum;
                memcpy(pCurrent,pFtr->GetLowLayerRoadSectionTurnList(),nSize);
                pCurrent += nSize;
            }
    ...
    {  
        ...
        iteratorOfFtr != pFtrContainer->end(); iteratorOfFtr++)
        {
            ...
            {
                CGIS_NetworkVertexV7::ST_GEO_VEDGE* pEdgeInfo = (CGIS_NetworkVertexV7::ST_GEO_VEDGE*)pCurrent;
                pEdgeInfo->nBreadthID = pFtr->GetEdgeBreadthID();
...
BOOL CNetworkFileDataV7::ReadNetwork(CFile* pFile)
{
	ASSERT(pFile);

	//读头
    StuNetworkFileStructV7 :: StuHeaderInfo* m_pFileHeadInfo = new StuNetworkFileStructV7 :: StuHeaderInfo;
	pFile->Read(m_pFileHeadInfo,sizeof(StuNetworkFileStructV7 :: StuHeaderInfo));

	int nCount = m_pFileHeadInfo->nMapCount;
	//---------------------------
	//读入地图索引
	StuNetworkFileStructV7::StuIndexInfoOfMap* pIndexInfoArray = NULL;		//地图索引信息
	pIndexInfoArray = new StuNetworkFileStructV7::StuIndexInfoOfMap[nCount];
	memset(pIndexInfoArray,0,sizeof(StuNetworkFileStructV7::StuIndexInfoOfMap)*nCount);
	pFile->Read(pIndexInfoArray,sizeof(StuNetworkFileStructV7::StuIndexInfoOfMap) * nCount);
    ...
    {        
        ...
        memcpy(pItem->pIndex,&pIndexInfoArray[i],sizeof(StuNetworkFileStructV7::StuIndexInfoOfMap));
...
BOOL CNetworkFileDataV7::ReadMap(CFile* pFile,stuMapItem* pMapItem)
{
	...
	StuNetworkFileStructV7::StuIndexInfoOfBreadth* pIndexInfoArray = 
        new StuNetworkFileStructV7::StuIndexInfoOfBreadth[nCount];
	pFile->Read(pIndexInfoArray,sizeof(StuNetworkFileStructV7::StuIndexInfoOfBreadth) * nCount);
	...
	{	
        ...
        pItem->pIndex = new StuNetworkFileStructV7::StuIndexInfoOfBreadth;
        pItem->pHeader = new StuNetworkFileStructV7::StuBreadthHeaderInfo;
		...
		memcpy(pItem->pIndex,&pIndexInfoArray[i],sizeof(StuNetworkFileStructV7::StuIndexInfoOfBreadth));
...
BOOL CNetworkFileDataV7::ReadBreadth(CFile* pFile,stuBreadthItem* pBreadthItem,int nLoadFlag)
{
	...
	char *pFBuf = NULL;
	{
...
#endif
                CGIS_NetworkRoadSectionV7 *pRoadSection = new CGIS_NetworkRoadSectionV7;
		...
		//--------------------------------------------------------------
		{
			...
			for(int i = 0;	i < nReadNum;	i++ )
			{
				...
				CGIS_NetworkVertexV7 *pVertex = new CGIS_NetworkVertexV7;
...
#ifdef ANALYSE_DATA
void CNetworkFileDataV7::CDataAnalyse::Initial()
{
...
void CNetworkFileDataV7::CDataAnalyse::DumpState()
{
...
BOOL CNetworkFileDataV7::Check(LPCTSTR strFilePath)
{
    //////////////////////////////////////////////////////////////////////////
    //加载索引ID数组
    {
...
//检查边界节点
BOOL CNetworkFileDataV7::CheckEdgeVertex(CGIS_NetworkVertexV7* pVertex)
{
	ASSERT(pVertex);
	ASSERT(pVertex->GetEdge());
		
	//取到相邻路段
	unsigned short nRutID = pVertex->GetEdgeRoadSectionID();
    ASSERT(nRutID != 0xffff);
    unsigned short nBreadthID = pVertex->GetEdgeBreadthID();
    ASSERT(nBreadthID != 0xffff);
	...
	CGIS_NetworkRoadSectionV7* pRoadSection = (*pRoadSectionContainer)[nRutID];
	...
	//////////////////////////////////////////////////////////////////////////	
	CGIS_NetworkVertexV7*   pSVtx = (*pVertexContainer)[pRoadSection->GetStartVtxID()];
	CGIS_NetworkVertexV7*   pEVtx = (*pVertexContainer)[pRoadSection->GetEndVtxID()];
	ASSERT(pSVtx);
	ASSERT(pEVtx);
